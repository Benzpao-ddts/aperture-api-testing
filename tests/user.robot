...
*** Variables ***
${SCHEMA_FILE}    resources/schema/user/getUser.json
${SCHEMA_FILE_POST_METHOD}      resources/schema/user/addUser.json
${SCHEMA_FILE_UPDATE_USER}      resources/schema/user/updateUser.json
...
Test API Status Code Should Be 200
    FOR    ${endpoint}    IN    @{LOOP_GET_ENDPOINTS_METHOD}
        Run Keyword And Continue On Failure     Check Endpoint Status    ${BASE_URL}${endpoint}
    END

Test API Response Times Less Than 2 Second

    FOR    ${endpoint}    IN    @{LOOP_GET_ENDPOINTS_METHOD}
        Run Keyword And Continue On Failure     Check Response Time    ${BASE_URL}${endpoint}
    END

Test Schema Validation
#    Check Schema     ${BASE_URL}
    FOR    ${endpoint}    IN    @{LOOP_GET_ENDPOINTS_METHOD}
        Run Keyword And Continue On Failure     Run Keyword If    'products/categories' != '${endpoint}'     Check Schema    ${BASE_URL}${endpoint}
...
Test Sorting ID Of User by Default
...
Test User ID Sorting: Max Value to Min Value
    [Documentation]    Verify if the `id` values in the response are sorted in ascending order.
    Create Session    api    ${BASE_URL}
...
Test User ID Sorting: Min Value to Max Value
    [Documentation]    Verify if the `id` values in the response are sorted in ascending order.
    Create Session    api    ${BASE_URL}
...
Test Add New Card
    Create Session    api    ${BASE_URL}
    ${response}=   Add Update Delete User     ${BASE_URL}${ALL_PRODUCT_URL}     POST
...
Test Update New User with PUT
    ${userid}=        Evaluate    7
    ${response}=   Add Update Delete User      ${BASE_URL}${ALL_PRODUCT_URL}/${userid}     PUT
    # Add condition to check response type and perform action
    Validate Response Schema Post Method        ${response}       ${SCHEMA_FILE_UPDATE_USER}


Test Update New Product with PATCH
    ${userid}=        Evaluate    3
    ${response}=   Add Update Delete User      ${BASE_URL}${ALL_PRODUCT_URL}/${userid}     PATCH
    # Add condition to check response type and perform action
    Validate Response Schema Post Method        ${response}       ${SCHEMA_FILE_UPDATE_USER}

Test Delete New Product
    ${userid}=        Evaluate    3
    ${response}=   Add Update Delete User      ${BASE_URL}${ALL_PRODUCT_URL}/${userid}     DELETE
    Validate Response Schema Post Method        ${response}       ${SCHEMA_FILE_POST_METHOD}
